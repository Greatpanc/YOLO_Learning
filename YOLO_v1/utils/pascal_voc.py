...
class pascal_voc(object):
    ...
    def prepare(self):
        ...
        if self.flipped:
            ...
            for idx in range(len(gt_labels_cp)):    # 循环一次处理一张图片
                ...
                gt_labels_cp[idx]['label'] =gt_labels_cp[idx]['label'][:, ::-1, :] # 7*7的区域水平翻转
                for i in range(self.cell_size):     # 对每一个区域中的Label x进行水平翻转
    ...
    def load_pascal_annotation(self, index):    # 从Annotation文件夹下的XML文件中加载图片标签中位置和边框信息PASCAL VOC format
